'''
파이썬 도서관리 프로그램
1. 도서등록(도서번호, 책 이름, 책 저자, 출판사)
2. 도서목록
3. 도서검색(제목, 저자, 출판사)
4. 도서삭제(해당하는 책 번호로 삭제)
5. 프로그램 종료

'''
book_mark = []; # 빈 리스트를 만들고 번호 이름 등 을 집어넣는다.
while True:
    print("=====================도서 관리 프로그램======================");
    print("1. 도서 등록");
    print("2. 도서 목록");
    ...
    if choice == 1:
        ...
        if search_type == 'n':
            ...
            author = input("검색 하려는 책 저자을 입력해 주세요 >> ");
            for item in book_mark:
                sec = item.get('author');
                if sec == author:
                    print("===========================================");
            ...
            publisher = input("검색 하려는 출판사을 입력해 주세요 >> ");
            for item in book_mark:
